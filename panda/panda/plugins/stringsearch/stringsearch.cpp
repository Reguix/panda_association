...
PPP_PROT_REG_CB(on_string_tainted);
}
...
PPP_CB_BOILERPLATE(on_string_tainted)
        ...
        for(int str_idx = 0; str_idx < num_strings; str_idx++) {
            ...
            if (sp.val[str_idx] == strlens[str_idx]) {
                ...
                PPP_RUN_CB(on_ssm, env, pc, in_memory ? match_addr : addr,
                           ...
                           in_memory);
                if(in_memory) {
                    PPP_RUN_CB(on_string_tainted, env, pc, in_memory ? match_addr : addr,
                               tofind[str_idx], strlens[str_idx],
