...
static inline char *get_exe_path(CPUState *env, target_ptr_t task_struct) {
    target_ptr_t exe_file;
    exe_file = get_exe_file(env, task_struct);
	return get_file_name(env, exe_file);
}
...
void fill_osiproc(CPUState *env, OsiProc *p, target_ptr_t task_addr) {
    ...
    p->exe_path = get_exe_path(env, task_addr);
